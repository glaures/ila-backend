...
/**
 * Freie Plätze einer Phase, aufgeschlüsselt nach Klassenstufe und Wochentag.
 * <p>
 * Beantwortet die Frage „wie viele Plätze stehen einem Kind dieser Stufe an diesem Tag noch
 * offen" – Grundlage für die Übersicht über den Belegungsproblemen im Frontend. Plätze eines
 * Kurses, der mehrere Stufen zulässt, erscheinen bewusst in jeder dieser Zeilen; die Zellen
 * dürfen deshalb nicht über die Stufen aufsummiert werden.
 */
@RestController
...
public class FreeSeatsService {
    ...
    public List<FreeSeatsDto> getFreeSeats(@PathVariable Long periodId) throws ServiceException {
        ...
        for (CourseGradeSeats row : courseBlockAssignmentRepository.findCourseGradeSeatsInPeriod(periodId)) {
            if (row.grade() == null || row.dayOfWeek() == null) continue;
            cells.computeIfAbsent(new Cell(row.grade(), row.dayOfWeek()), c -> new Accumulator())
                    .add(row);
        }
        ...
        result.sort(Comparator.comparingInt(FreeSeatsDto::grade)
                .thenComparing(FreeSeatsDto::dayOfWeek));
        return result;
    }

    private record Cell(int grade, DayOfWeek dayOfWeek) {
    }
